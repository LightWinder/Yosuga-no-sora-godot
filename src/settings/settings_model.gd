...
static func defaults() -> Dictionary:
	...
	return {
		...
		"read_color": true,
		"screen_effect": true,
		# Keep the source CONFIG.readSkip polarity: true means the
		# "允许跳过未读文本" setting is off in the UI.
		"read_skip": true,
...
static func apply_patch(current: Dictionary, patch: Dictionary) -> Dictionary:
	var result := current.duplicate(true)
	for key in patch:
		# Confirmation controls emit one changed flag at a time. Preserve the
		# remaining source flags instead of replacing the complete map.
		if key == "confirmations" and patch[key] is Dictionary:
			var confirmations: Dictionary = result.get("confirmations", {}).duplicate(true)
