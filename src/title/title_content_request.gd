class_name TitleContentRequest
extends RefCounted
...
static func create(catalog: StringName, entry: StringName) -> TitleContentRequest:
	var request := TitleContentRequest.new()
	request.catalog_id = catalog
	request.entry_id = entry
	return request


static func for_album(group_id: StringName, variant: TitleAlbumVariant) -> TitleContentRequest:
...
static func for_music(track: TitleMusicTrack) -> TitleContentRequest:
...
static func for_memory(memory: TitleMemoryEntry) -> TitleContentRequest:
	var request := create(&"memories", memory.entry_id if memory != null else &"")
	if memory != null:
		request.kind = TitleContentRequest.RequestKind.MEMORY_VIDEO if memory.is_video() else TitleContentRequest.RequestKind.MEMORY_ADV
...
static func for_voice(favorite: VoiceFavorite) -> TitleContentRequest:
