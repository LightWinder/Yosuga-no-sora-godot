class_name AppreciationContentManifest
extends RefCounted


const PATH := "res://assets/manifests/appreciation_content_manifest.json"

var schema_version: int = 0
var album_groups: Array[AppreciationAlbumGroup] = []
var music_tracks: Array[AppreciationMusicTrack] = []
var memory_entries: Array[AppreciationMemoryEntry] = []
var source_contract: Dictionary = {}
var load_error: String = ""


static func load_default() -> AppreciationContentManifest:
	var result := AppreciationContentManifest.new()
...
func _decode_album_group(raw: Dictionary) -> AppreciationAlbumGroup:
	var group := AppreciationAlbumGroup.new()
...
func _decode_album_card(raw: Dictionary) -> AppreciationAlbumCard:
	var card := AppreciationAlbumCard.new()
	...
	if raw_variants is Array:
		for value in raw_variants:
			if value is Dictionary:
				card.variants.append(AppreciationAlbumVariant.from_dictionary(value as Dictionary))
	return card


func _decode_music(raw: Dictionary) -> AppreciationMusicTrack:
	var track := AppreciationMusicTrack.new()
...
func _decode_memory(raw: Dictionary) -> AppreciationMemoryEntry:
	var memory := AppreciationMemoryEntry.new()
	...
	memory.kind = AppreciationMemoryEntry.EntryKind.VIDEO if str(raw.get("kind", "adv")) == "video" else AppreciationMemoryEntry.EntryKind.ADV
