class_name AppreciationAlbumViewer
extends Control


signal closed
signal variant_changed(variant: AppreciationAlbumVariant)

var _card: AppreciationAlbumCard
var _profile: ProfileData
var _variants: Array[AppreciationAlbumVariant] = []
var _index := 0
...
func configure(card: AppreciationAlbumCard, profile: ProfileData) -> void:
	_card = card
	_profile = profile
...
func open_variant(variant: AppreciationAlbumVariant = null) -> bool:
...
func next_variant() -> AppreciationAlbumVariant:
	if _variants.is_empty():
		return null
...
func previous_variant() -> AppreciationAlbumVariant:
	if _variants.is_empty():
		return null
	_index = posmod(_index - 1, _variants.size())
	_refresh()
...
func current_variant() -> AppreciationAlbumVariant:
