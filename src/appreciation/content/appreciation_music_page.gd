class_name AppreciationMusicPage
...
const TRACK_SCENE := preload("res://src/appreciation/ui/appreciation_music_button.tscn")

var _manifest: AppreciationContentManifest
var _selected: AppreciationMusicTrack
var _playing := false
...
func configure(manifest: AppreciationContentManifest) -> void:
	_manifest = manifest
	if is_inside_tree():
		_refresh()
...
func selected_track() -> AppreciationMusicTrack:
	return _selected
...
func _refresh() -> void:
	...
	for row in 7:
		for column in 3:
			...
			if index >= _manifest.music_tracks.size():
				continue
...
func _add_track_button(index: int, track: AppreciationMusicTrack) -> void:
	var button := TRACK_SCENE.instantiate() as AppreciationMusicButton
	button.name = "Track_%s" % str(track.track_id).validate_node_name()
	button.configure(index, track)
	button.pressed.connect(_select_track.bind(track))
	_track_list.add_child(button)


func _select_track(track: AppreciationMusicTrack) -> void:
	...
	_status.text = ""
	_refresh_selected_buttons()
	content_requested.emit(AppreciationContentRequest.for_music(track))
	status_changed.emit(_status.text)


func _refresh_selected_buttons() -> void:
	for child in _track_list.get_children():
		var button := child as AppreciationMusicButton
		if button == null:
			continue
