...
func play_page_transition(content: Control, direction: int) -> void:
	reset_page_transition(content)
	content.position = Vector2(float(signi(direction)) * 72.0, 0.0)
	...
	_page_tween.tween_property(content, "position", Vector2.ZERO, 0.22)
	_page_tween.tween_property(content, "modulate:a", 1.0, 0.18)


func reset_page_transition(content: Control) -> void:
