...
The design resolution is 1920×1080. The current development window starts at 2560×1440 and scales at a fixed 16:9 aspect ratio. The Title background independently covers the viewport. Its cloud overlay radially maps the complete transparent `AAA.png` around design position (1280, 864), using an adjustable logarithmic radius and angular coordinate. A single scene-owned CanvasItem renders the clouds; there are no individual cloud instances or respawns. A normalized phase clock avoids hourly shader-time resets, and an artwork-space sky mask keeps clouds off the scenery. Projection follows the background's cover crop. The content root preserves its aspect ratio in ultrawide, 4:3, and portrait windows. Desktop 16:9 is not reduced unnecessarily; mobile layouts additionally respect the system safe area, with a conservative fallback when unavailable.

## Verification

Title now uses the whole-texture radial cloud implementation. `tests/cloud_test.tscn` retains ordinary vertical-loop inspection, while `tests/cloud_radial_test.tscn` previews the same cloud field used by Title. Both use the supplied `AAA.png`. Texture-edge defects are recorded for later artwork repair and no longer block implementation, as requested. See [cloud validation and tuning](docs/cloud_loop_validation.md).
...
- `src/title/`: the Title route and reusable menu components. `title_screen.tscn` owns the fixed background, projected cloud field, character variants, primary/bonus menu buttons, and footer chrome. The cloud field is one scene-owned fullscreen Control; its CanvasItem shader maps a single repeating cloud texture by angle and logarithmic radius, with a static sky mask. A small local controller advances only the normalized texture phase. The Title controller only synchronizes visibility, focus, signals, save state, and transitions. The Load route uses a lightweight feature host, and Title reaches Settings only through the `settings` route.
